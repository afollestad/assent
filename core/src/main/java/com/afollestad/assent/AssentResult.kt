...
class AssentResult(
  ...
  internal constructor(
    ...
    resultsMap = results.map { (permission, isGranted) ->
      permission.toPermission()
        .withGrantResult(isGranted, shouldShowRationale)
    }.toMap()
  )
  ...
  override fun equals(other: Any?): Boolean =
    other is AssentResult && other.resultsMap == resultsMap

  override fun toString(): String =
    resultsMap.entries.joinToString(separator = ", ") { "${it.key} -> ${it.value}" }
}
