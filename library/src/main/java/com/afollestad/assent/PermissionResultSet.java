...
public class PermissionResultSet {
  ...
  public Map<String, Boolean> getGrantedMap() {
    ...
    for (String perm : getPermissions()) map.put(perm, isGranted(perm));
    return map;
  }

  public String[] getPermissions() {
    String[] perms = new String[mResults.length];
    for (int i = 0; i < perms.length; i++) perms[i] = mResults[i].getPermission();
    return perms;
  }
  ...
  public boolean isGranted(@NonNull String permission) {
    synchronized (mResults) {
      for (PermissionResult result : mResults) {
        if (result.getPermission().equals(permission)) return result.isGranted();
      }
      return false;
    }
  }
