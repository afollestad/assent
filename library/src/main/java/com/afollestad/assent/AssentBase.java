...
class AssentBase {
  ...
  protected static String join(String[] array) {
    ...
    // Estimate how long the string will be for the initial capacity, doubles in capacity when limit is reached
    StringBuilder sb =
        new StringBuilder((array[0].length() * array.length) + 4 + (2 * (array.length - 1)));
    sb.append("[ ");
    for (int i = 0; i < array.length; i++) {
      if (i > 0) sb.append(", ");
      sb.append(array[i]);
  ...
  protected static void LOG(
      @NonNull String tag, @NonNull String message, @Nullable Object... args) {
    if (!LOGGING_ENABLED) return;
    if (args != null) Log.d(tag, String.format(message, args));
